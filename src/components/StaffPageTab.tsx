...
function createData( fname: string, lname: string, email: string) {
...
export default function StaffPageTab() {
    ...
    const handleAddGradRow = (f: any, l: any, e: any) =>{
      ...
      const handleDemoteButton = (first: any, last: any, email: any, idx: any) =>{
        let apiBaseUrl: string = '/api/enrolled_course/change_role';
        let payload : object = {
            "user_id": 3,
            "course_id": 1,
            "role": "GRADER",
    ...
    useEffect(() => {
        ...
        api.get(apiBaseUrl, {params: {course_id: 1, roles: "GRADER"}})
            .then((response) => {
                arr2 = response.data.result.filter((user:any) => user.user_info);
                let stu = []
    ...
    const renderCells = (row:any, index:any) => {
        return (
        ...
        const renderCells2 = (row:any, index:any) => {
            return (
